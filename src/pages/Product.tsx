...
export default function Product() {
  ...
  // لما تضغط على لون تتغير الصورة للصورة المربوطة بهذا اللون
  const colorImage = currentColor && product.colorImages?.[currentColor];
  const mainImage = colorImage || product.image;
  
  // gallery: صور اللون المختار أو الصور العامة
  const gallery = colorImage 
    ? [colorImage, ...(product.images || []).filter(img => img !== colorImage)].filter(Boolean)
    : (product.images?.length ? product.images : [mainImage]).filter(Boolean);
