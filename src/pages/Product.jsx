...
export default function Product() {
  ...
  if (!product) return (
    <PageHead title="المنتج غير موجود" crumb="الرئيسية / المتجر / غير موجود">
      <section className="section" style={{ paddingTop: 60 }}>
        <div style={{ textAlign: "center" }}><Icon3D name="bag" size={68} /><p style={{ marginTop: 14 }}>لم يتم العثور على هذا المنتج</p></div>
  ...
  const mainImageData = product.colorImages?.[currentColor] || product.image;
                ...
                <div className="colors-row">
                  {product.colors.map((c, i) => (
                    <button
                      ...
                      style={{ background: c }}
                      onClick={() => { setSelectedColor(i); setActiveThumb(0); }}
                    />
                  ))}
                </div>
              </div>
