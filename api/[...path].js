...
// config route moved to bottom with heroImage support
...
app.post('/api/admin/settings', auth, async (req,res) => {
  try {
    ...
    // حفظ في Supabase كإعداد
    await sb('settings', {
      method: 'POST',
      body: {key: 'hero_image', value: heroImage}
    }).catch(async () => {
      // إذا الجدول ما موجود، نرجع OK
    });
    res.json({ok: true, heroImage});
  } catch(e) { res.status(500).json({error:e.message}); }
});
...
app.get('/api/config', async (req,res) => {
  try {
    // حاول تجيب من Supabase
    let heroImage = null;
    try {
      const rows = await sb('settings', {filter: '&key=eq.hero_image'});
      if(rows && rows[0]) heroImage = rows[0].value;
    } catch(e) {}
